...
export default function useTaskStats(tasks, users, currentEmployeeId){
  return useMemo(()=>{
    const statusCounts={}; const priorityCounts={};
    Object.keys(STATUS_META).forEach(k=> statusCounts[k]=0);
    Object.keys(PRIORITY_META).forEach(k=> priorityCounts[k]=0);
    const today=new Date(); today.setHours(0,0,0,0);
    const workloadMap={}; const overdueList=[]; let completed=0, inProgress=0, overdue=0;
    tasks.forEach(task=>{
      ...
      workloadMap[key].count+=1; if(task.status===5) workloadMap[key].completed+=1;
    ...
    const workload = Object.entries(workloadMap).map(([employeeId,w])=>{
      ...
      return { employeeId, name: u? `${u.firstName} ${u.lastName}`: employeeId==='Unassigned'?'Unassigned':employeeId, photo:u?.photo||'', count:w.count, completed:w.completed, pending:w.count-w.completed, pct: w.count? Math.round(w.completed/w.count*100):0 };
    }).sort((a,b)=> b.count-a.count);
    const monthly=[];
    const now=new Date();
    for(let i=11;i>=0;i--){ const d=new Date(now.getFullYear(), now.getMonth()-i,1); monthly.push({ name: d.toLocaleString('en',{month:'short'}), created:0 }); }
    tasks.forEach(task=>{
      const created=new Date(task.createdAt);
      const monthIdx=(now.getFullYear()-created.getFullYear())*12 + (now.getMonth()-created.getMonth());
      if(monthIdx>=0 && monthIdx<12) monthly[11-monthIdx].created+=1;
    });
    return {
      total: tasks.length, completed, inProgress, overdue,
      ...
      monthly, workload, overdueList: overdueList.sort((a,b)=> b.daysOver-a.daysOver),
    };
