...
export default function TaskStatsBoard({ stats, onView }){
  ...
  const tiles=[
    { icon:<FiClipboard size={22}/>, label:'Total', value:stats.total, sub:`${stats.myTasks} yours`, color:'#14286D', tint:'#E2E7F5'},
    ...
    { icon:<FiUsers size={22}/>, label:'My Tasks', value:stats.myTasks, sub:'assigned', color:'#7C3AED', tint:'#F1E9FC'},
  ];
  if(stats.total===0) return <Paper sx={{ p:6, textAlign:'center', borderRadius:3 }}><FiBarChart2 size={48} color="#C9D6EE"/><Typography fontWeight={700} sx={{ mt:1 }}>No statistics yet</Typography></Paper>;
  return (
    <Box>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={8}>
          <Paper sx={{ p:2, borderRadius:3, border:'1px solid #E8EEF9' }}>
            <Typography fontWeight={700} sx={{ mb:1 }}>Tasks Created (12 months)</Typography>
            <Box sx={{ height:280 }}>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p:2, borderRadius:3, border:'1px solid #E8EEF9' }}>
            <Typography fontWeight={700} sx={{ mb:1 }}>Workload by Assignee</Typography>
            <Box sx={{ height:300 }}>
              <ResponsiveContainer width="100%" height="100%">
      ...
      {stats.overdueList.length>0 && (
        <Paper sx={{ mt:2, p:2, borderRadius:3, border:'1px solid #FECACA', bgcolor:'#FFF7F7' }}>
          ...
          <Box sx={{ display:'flex', gap:1, flexWrap:'wrap' }}>
            {stats.overdueList.slice(0,8).map(t=> <Chip key={t.id} label={`${t.title} • ${t.daysOver}d`} onClick={()=> onView(t)} size="small" color="error" variant="outlined" sx={{ cursor:'pointer' }}/>)}
          </Box>
        </Paper>
      )}
    </Box>
