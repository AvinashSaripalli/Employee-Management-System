...
import { XAxis, YAxis, Tooltip, ResponsiveContainer, Area,AreaChart, CartesianGrid, PieChart, Pie,Cell,Text, BarChart,Bar,Legend,Sector, ComposedChart } from 'recharts';
...
const HrDashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Source of truth = Company Structure departments; fallback to users-by-departments
        let deptNames = [];
        try {
        ...
        // also include any dept from users not in structure (legacy)
  ...
  <ResponsiveContainer width={775} height={350}>
    ...
    >
      ...
      <CartesianGrid strokeOpacity={0.2} horizontal={false} />
      <Tooltip cursor={{ fill: "transparent" }} />
      <Bar dataKey="indepartment" name={departmentAlias} barSize={30}>
        {departmentOrder.map((entry, index) => (
          <Cell key={`cell-${index}`} fill={colorsbar[index % colorsbar.length]} />
        ))}
      </Bar>
