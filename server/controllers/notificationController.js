  ...
  try {
    ...
    if (activeShift) {
      notifications.push({
        ...
        detail: `Clocked in at ${activeShift.clockInTime ? activeShift.clockInTime.slice(0, 5) : 'today'}. Remember to conclude your shift and file your daily work report.`,
      ...
      if (!submittedDates.has(dateStr)) {
        ...
        if (isPast) {
          notifications.push({
            ...
            badge: 'Overdue',
            severity: 'urgent',
            title: `Work Report Overdue (${dateStr})`,
    ...
    for (const task of activeTasks) {
      ...
      if (isOverdue) {
        notifications.push({
          ...
          detail: `Deadline passed on ${task.deadline.toISOString().slice(0, 10)}. Please review and update task status.`,
          createdAt: deadline,
        ...
        notifications.push({
          ...
          detail: `Due on ${task.deadline.toISOString().slice(0, 10)}. Don't forget to submit your updates.`,
        ...
        notifications.push({
          ...
          detail: task.description ? task.description.slice(0, 85) : 'Task assigned to you in progress',
    ...
    if (isAdmin || isSupervisor) {
      ...
      for (const leave of pendingLeaves) {
        notifications.push({
          ...
          detail: `${leave.employee_name || 'Employee'} (${leave.department}) requested ${leave.leave_type} (${leave.days || 1} day(s)) from ${leave.start_date} to ${leave.end_date}.`,
    ...
    for (const leave of personalLeaves) {
      ...
      notifications.push({
        ...
        detail: `Your ${leave.leave_type} request for ${leave.start_date} has been ${leave.status.toLowerCase()}.`,
      ...
      for (const rep of recentReports) {
        if (rep.employeeId !== employeeId) {
          notifications.push({
            ...
            detail: `${rep.department} · ${rep.taskName} (${rep.hoursWorked}h) on ${rep.date}.`,
    ...
    for (const msg of recentMessages) {
      notifications.push({
        ...
        detail: msg.content ? (msg.content.length > 70 ? `${msg.content.slice(0, 70)}...` : msg.content) : 'Sent an attachment',
